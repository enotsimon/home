...
export default class Agent {
  ...
  exchange_symbol(prop, new_symbol) {
    const length_was = this[prop].length
  ...
  advance_array(symbol_class) {
    const first = symbol_class.shift()
  ...
  do_something_with_symbol(prop, symbol, fun) {
    ...
    // omg!
    const length_was = this[prop].length
    const ret = this[prop][fun](symbol)
    const diff = this[prop].length - length_was
  ...
  calc_valuables(prop) {
    const stat = this.stat[prop].counters
    const max_count = 0
