...
export default class PlanetsFocus extends BasicDrawer {
  ...
  redraw() {
    ...
    if (--this.focus_change_tick <= 0) {
      ...
      if (!this.forced_focus) {
        const cur_index = this.bodies.indexOf(this.focused_body)
  ...
  update_matrix_by_focus() {
    ...
    const coords = this.calc_coords_recursively(this.focused_body)
  ...
  init_bodies() {
    ...
    this.star = this.init_body('star', null, 0, 10, 0, 0.5)
    ...
    this.moon21 = this.init_body('moon21', this.planet2, 8, 2, 3, 0.1)
  ...
  init_body(name, parent, orbital_radius, radius, orbital_speed, rotation_speed) {
    const body = new StellarBody(name, parent, orbital_radius, radius, orbital_speed, rotation_speed)
    this.init_body_graphics(body, parent)
    const parent_graphics = parent ? parent.base_container : this.matrix
  ...
  init_body_graphics(body, parent) {
    ...
    ([1, 2, 3]).forEach(i => {
      const coords = Util.from_polar_coords(i * 2 * Math.PI / 3, body.radius / 2)
...
class StellarBody {
  constructor(name, parent, orbital_radius, radius, orbital_speed, rotation_speed, orbital_angle = false, angle = false) {
    ...
    this.orbital_angle = orbital_angle || 2 * Math.PI * Math.random()
    this.angle = angle || 2 * Math.PI * Math.random()
  }
}
