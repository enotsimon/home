...
export default class DensityMap {
  ...
  random_point(initial = false) {
    return { x: 2 * Math.random() - 1, y: 2 * Math.random() - 1, initial }
  }
  ...
  generate(count, count_basic = 10) {
    ...
    // basic points
    while (count_basic) {
      const point = this.random_point(true)
    ...
    while (this.points.length < count) {
      const point = this.random_point(false)
      if (!this.check_in_circle(point)) {
  ...
  check_average_distance(point) {
    const value = Util.find_min_and_max(this.points, (p) => Util.distance(p, point)).min
    // console.log('value', value);
    // its like some little chanse for far points
    const rand = value * Math.pow(Math.random(), 100)
  ...
  draw_naive(scale, graphics) {
    ...
    this.points.forEach(point => {
      ...
      const color = point.initial ? [125, 255, 0] : [125, 0, 0]
  ...
  draw_density_grid(scale, graphics, size = 100) {
    ...
    for (let y = -1; y < 1; y += step) {
      for (let x = -1; x < 1; x += step) {
        const grid_point = { x, y, density: 0 }
    ...
    const max_density = Util.find_min_and_max(grid_points, (p) => p.density).max
    grid_points.forEach(point => {
      const channel = Util.normalize_value(point.density, max_density, 255, 0, 20)
