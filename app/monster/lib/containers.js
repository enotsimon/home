...
export function container_dispatch_init(id) {
...
// item or id_item???
export function container_dispatch_add_item(id_container, id_item) {
  let containers = game.store.getState().containers;
...
// item or id_item???
export function container_dispatch_remove_item(id_container, id_item) {
  let containers = game.store.getState().containers;
  let container = containers[id_container];
  if (!container) {
    // throw?
...
//
export function container_reduce_init(state, id_container) {
  let new_state = {...state};
  new_state[id_container] = new Container(id_container)
  return new_state;
}

export function container_reduce_add_item(state, id_container, id_item) {
  let new_state = {...state};
