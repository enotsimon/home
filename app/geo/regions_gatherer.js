...
export default class RegionsGatherer {
  static gather_regions(diagram) {
    const regions_mode = 'rrt_leafs' // 'closest'
    ...
    if (regions_mode == 'rrt_leafs') {
      const starters = diagram.cells.filter(cell => cell.rrt_links.length != 2)
      starters.forEach(cell => {
        const region = { color: MapDrawer.random_color(), cells: [cell] }
      ...
      if (regions_mode == 'closest') {
        ...
        const generations_per_regions = 5
        get_linked_cells_func = (cell) => {
          const my_region_index = Math.floor(cell.generation / generations_per_regions)
  ...
  static diagram_collect_regions(diagram, cell, get_linked_cells_func) {
    ...
    const linked_cells = get_linked_cells_func(cell) // cell.closest_backlinks.concat([cell.closest_link]);
    let region
  ...
  static gather_all_parents_to_my_region(cell) {
    const parent = RRTDiagram.get_parent(cell)
  ...
  static recursively_merge_small_regions(diagram) {
    const small_regions = diagram.regions.filter(region => region.cells.length < regions_min_cells)
    if (!small_regions.length) {
      return
    }
    // smallest first. actually, we need only first one
    const region = RegionsGatherer.sort_regions_by_size(small_regions)[0] // small_regions.shift();
    const linked_regions = RegionsGatherer.gather_all_linked_regions(region)
    RegionsGatherer.sort_regions_by_size(linked_regions)
    const target_region = linked_regions[0]
