...
export default class Geo {
  constructor(diagram, rrt, map_drawer) {
    ...
    this.rrt = rrt
    const max_branch_order = Util.find_min_and_max(this.rrt.nodes, node => node.branch_order).max
    console.log('WE GONNA remove_some_links_and_recalc_all', this.rrt_regions - 1)
    const deleted_links = this.rrt.remove_some_links_and_recalc_all(this.rrt_regions - 1, max_branch_order - 10, max_branch_order - 2)
    const centers = this.rrt.nodes.filter(node => node.generation == 0)
  ...
  generate_sea(sea_ratio) {
    const sea_cells_threshold = Math.floor(sea_ratio * this.diagram.cells.length)
    let count_sea_cells = 0
    for (const i in this.heights_stat()) {
  ...
  generate_rocks_by_height() {
    const max_height = Util.find_min_and_max(this.rrt.nodes, e => e.height).max
  ...
  calc_fertility() {
    ...
    const fertility_multiplier = Math.round(square_sum / this.diagram.cells.length)
    ...
    this.diagram.cells.forEach(cell => {
      ...
      const square = Util.convex_polygon_square(cell.nodes)
  ...
  geo_types_by_fertility() {
    ...
    const min = stats[0]
    const max = stats[Math.floor(0.95 * stats.length)] // 95% persentile cause there's big gaps in the end
    const geo_type_fertility_thresholds = {
    ...
    this.diagram.cells.forEach(cell => {
      ...
      for (const i in geo_type_fertility_thresholds) {
  ...
  height_smooth(max_diff_coef = 0.3) {
    const h = Util.find_min_and_max(this.rrt.nodes, node => node.height)
    const max_diff = max_diff_coef * (h.max - h.min)
    let changed = 0
    do {
      changed = 0
      this.diagram.cells.forEach(cell => {
        const max_height = Util.find_min_and_max(cell.links, c => c.height).max
        const height_to_max_diff = max_height - max_diff
  ...
  generate_heights_by_rrt_generations(peak_max_height = 200, peak_min_height = 200, step = 2) {
    const peak_height = Util.rand(peak_min_height, peak_max_height)
