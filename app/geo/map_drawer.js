...
export default class MapDrawer {
  ...
  highlight_cell_under_cursor(cell) {
    ...
    const polygon = cell.nodes.map(node => new PIXI.Point(node.x, node.y))
    const border_graphics = new PIXI.Graphics()
    ...
    const inner_graphics = new PIXI.Graphics()
  ...
  highlight_local_minimums() {
    this.diagram.cells.forEach(cell => {
      ...
      if (cell.links.every(link => link.height > cell.height)) {
        const graphics = MapDrawer.draw_polygon(cell.nodes, [255, 0, 125])
        graphics.alpha = 0.5
  ...
  print_text_for_each_cell(fn) {
    this.diagram.cells.forEach(cell => {
      const msg = fn(cell)
      const text = new PIXI.Text(msg, { fontFamily: 'Arial', fontSize: 10, fill: 0xff1010 })
      text.x = cell.x + 2
      text.y = cell.y
  ...
  init_heignts() {
    ...
    let max_height = this.diagram.cells[0].height
    this.diagram.cells.forEach(cell => {
      if (cell.height < min_height) min_height = cell.height
      if (cell.height > max_height) max_height = cell.height
    })
    this.diagram.cells.forEach(cell => {
      const c = Util.normalize_value(cell.height, max_height, 255, min_height, 0)
      const graphics = MapDrawer.draw_polygon(cell.nodes, [0, c, 0])
      this.layers.heights.addChild(graphics) // z-index?
    })
  }

  init_geo_types() {
    const geo_types_colors = {
      sea: [0, 50, 100],
      rock: [60, 60, 50],
    ...
    this.diagram.cells.forEach(cell => {
      ...
      const fill_color = geo_types_colors[cell.geo_type]
      const graphics = MapDrawer.draw_polygon(cell.nodes, fill_color)
  ...
  init_rrt() {
    const color = ([0, 125, 255]).sort((e1, e2) => 0.5 - Math.random())
    const graphics = new PIXI.Graphics()
    this.layers.rrt_links.addChild(graphics)
    const bla = Util.find_min_and_max(this.rrt.nodes, (e) => e.height)
  ...
  init_dark_mode() {
    this.diagram.cells.forEach(cell => {
      const color = cell.geo_type == 'sea' || cell.geo_type == 'lake' ? [0, 0, 100] : [0, 0, 0]
      const graphics = MapDrawer.draw_polygon(cell.nodes, color)
  ...
  static draw_arrow(from, to, graphics, line_width, color) {
    ...
    const angle = Util.to_polar_coords(from.x - to.x, from.y - to.y).angle
    const p1 = Util.from_polar_coords(angle - Util.radians(15), 7 * line_width)
    const p2 = Util.from_polar_coords(angle + Util.radians(15), 7 * line_width)
  ...
  static draw_smoothed_polygon(graphics, polygon, center, water_color) {
    ...
    const mid_radius = polygon.reduce((sum, e) => sum + Util.distance(e, center), 0) / polygon.length
    polygon = polygon.filter((node, i) => {
      const next_i = (i + 1 == polygon.length) ? 0 : i + 1
      const next_node = polygon[next_i]
      return Util.distance(node, next_node) >= 0.2 * mid_radius
    })

    graphics.beginFill(Color.to_pixi(water_color))
    const fl_mid = { x: (polygon[0].x + Util.last(polygon).x) / 2, y: (polygon[0].y + Util.last(polygon).y) / 2 }
    graphics.moveTo(fl_mid.x, fl_mid.y)
    Util.for_all_consecutive_pairs(polygon, (cur, next) => {
      const pc_mid = { x: (next.x + cur.x) / 2, y: (next.y + cur.y) / 2 }
  ...
  static draw_broken_line_between_two_cells(c1, c2, graphics, diagram, color, width = 1) {
    const mid_point = MapDrawer.two_cells_edge_midpoint(diagram, c1, c2)
  ...
  static two_cells_edge_midpoint(diagram, c1, c2) {
    let my_edge
    for (const i in c2.halfedges) {
      const edge = diagram.edges[c2.halfedges[i]]
